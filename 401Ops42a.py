...
if "-" not in port_range or len(port_range.split("-")) != 2 or not all(x.isdigit() for x in port_range.split("-")):
...
if resp in ['1', '2', '3']:
    ...
    try:
        ...
        if protocol in scanner[ip_addr]:
            print(Fore.GREEN + "Open Ports:", list(scanner[ip_addr][protocol].keys()) + Style.RESET_ALL)
        else:
